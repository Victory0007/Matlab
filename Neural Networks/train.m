function [J, newWeights] = train(weights, layer_sizes, X, y_new, lambda, alfa)
    ...
    for i = 2:size(layer_sizes,2)
        %disp(size(a{i-1}))
        %disp(size(weights{i-1}'))
        z{1,i} = a{i-1}*weights{1,i-1}';
        a{1,i} = sigmoid(z{1,i});
    ...
    for i = size(layer_sizes,2):-1:2
        if i == size(layer_sizes,2)
            ...
            %This all gives the upstream gradient for every layer in the
            %neural network
            %Error of the next layer multiplied by the error of the weight
            %matix of the current layer(Excluding the bias weights)
            %element multiplication by the sigmoid gradient of the current layer.
    ...
    %% GRADIENT DESCENT
    for i = 1:(size(layer_sizes,2)-1) 
        %Why didnt they do l2 regularization here
